  ...
  const SessionItem = ({ session }: { session: Session }) => {
    ...
    return (
      <div className={`group relative rounded-xl transition-all duration-200 ${isActive ? 'bg-white/[0.06]' : 'hover:bg-white/[0.03]'}`}>
        ...
        {/* Delete button */}
        <button
          onClick={(e) => { e.preventDefault(); setSessionToDelete(session); }}
          disabled={deletingId === session._id}
          className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 hover:bg-red-500/20 text-white/20 hover:text-red-400 transition-all"
        >
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="fixed lg:relative z-50 h-dvh flex flex-col overflow-hidden shrink-0"
      >
        <div className="flex flex-col h-full w-[260px] bg-[#080808] border-r border-white/[0.06]">
          ...
          <div className="px-3 py-3 border-t border-white/[0.04]">
            ...
            <div className="text-center mt-2 text-[8px] text-white/10 font-mono tracking-widest uppercase">
              Mars AI v2.0 — Auth: HUNAIN
            </div>
          </div>
        </div>
      </motion.aside>
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <div>
                  <h2 className="text-base font-black uppercase tracking-tight text-white mb-1">Delete Mission?</h2>
                  <p className="text-xs text-white/40 leading-relaxed max-w-[200px] mx-auto">
                    "<span className="text-white/60">{sessionToDelete.title}</span>" will be permanently erased.
                  </p>
                </div>
